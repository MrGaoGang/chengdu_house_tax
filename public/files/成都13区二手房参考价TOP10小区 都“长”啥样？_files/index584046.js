...
P.innerText="支付",P.className="weui-btn weui-btn_primary",y.on(P,"tap",function(){
...
I.show());
})):e.err_msg.indexOf("cancel")>-1?I.show():window.weui.alert("充值失败，请稍后重试"));
});
},250)):(I.hide(),B.show("生成订单中"),l({
...
case 202602:
window.weui.alert("你已购买过该文章，无需重复购买",j);
break;
...
case 202617:
window.weui.alert("青少年模式下不可付费"),R.setSum(232209,0,1);
break;

default:
T.report110809(14),window.weui.alert("订单创建失败，请稍后重试");
}
},
error:function(){
...
if(d)return void window.weui.alert("暂不支持Google Play支付，前往微信官网下载最新版微信，即可正常付费");
if(!i){
if(!e&&g)return;
...
if(!e&&1*n.dataset.ready===0)return!i&&B.show("生成订单中"),new Date-h>6e4?(B.hide(),
window.weui.alert("支付超时，请稍后重试"),void(g=!1)):setTimeout(S,100,{
currentTarget:n
},!0);
if(T.reportPayAppmsg(9),1===window.is_transfer_msg)return window.weui.confirm("此内容来自于迁移前的帐号，不支持付费",{
buttons:[{
type:"default",
label:"确定"
}]
}),void(g=!1);
if(window.is_teenager)return window.weui.alert("青少年模式下不可付费"),R.setSum(232209,0,1),
void(g=!1);
...
t.innerText="知道了",t.className="weui-btn weui-btn_default";
var o=function(){
...
(X||navigator.userAgent.indexOf("Br_trunk")>-1)&&f.isIOS&&(s||d)&&!p?(console.info("[HEVC代理] 当前版本可以启用HEVC代理"),
